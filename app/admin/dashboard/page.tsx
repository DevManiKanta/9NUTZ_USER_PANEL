...
export default function AdminDashboard() {
  ...
  // NOTE: include token so we can set Authorization header when calling admin endpoints
  ...
  const [productForm, setProductForm] = useState({
    ...
    rating: 0,
    reviews: 0,
    quantityPrices: [{ quantity: "", price: 0 }] as { quantity: string; price: number }[]
  });
  ...
  const handleEditProduct = (productId: string) => {
    ...
    if (product) {
      setProductForm({
        ...
        offerPrice: (product as any).offerPrice ?? 0,
        categoryId: (product as any).categoryId ?? (product as any).category_id ?? "",
        images: Array.isArray((product as any).images) && (product as any).images.length ? (product as any).images : (product as any).image ? [(product as any).image] : [""],
        imageFiles: [],
        stock: (product as any).stock ?? (product as any).inventory ?? 0,
        ...
        quantityPrices:
          Array.isArray((product as any).quantityPrices) && (product as any).quantityPrices.length
            ? (product as any).quantityPrices.map((q: any) => ({ quantity: q.quantity ?? q.size ?? String(q.size), price: Number(q.price ?? product.offerPrice ?? product.price) }))
            : [{ quantity: (product as any).weight ?? "", price: (product as any).offerPrice ?? (product as any).price ?? 0 }]
      });
  ...
  const addQuantityRow = () => {
    setProductForm((prev) => ({ ...prev, quantityPrices: [...prev.quantityPrices, { quantity: "", price: 0 }] }));
  };
  const updateQuantityRow = (idx: number, key: "quantity" | "price", val: any) => {
    setProductForm((prev) => ({
      ...prev,
      quantityPrices: prev.quantityPrices.map((r, i) => (i === idx ? { ...r, [key]: key === "price" ? Number(val) : val } : r))
    }));
  };
  const removeQuantityRow = (idx: number) => {
    setProductForm((prev) => ({ ...prev, quantityPrices: prev.quantityPrices.filter((_, i) => i !== idx) }));
  };

  // handleProductSubmit - uses token from useAuth() above (so Authorization header will be sent)
  const handleProductSubmit = async (e: React.FormEvent) => {
    e.preventDefault();

    const authToken = token ?? null; // token provided by AuthContext
    ...
    try {
      ...
      if (productForm.imageFiles && productForm.imageFiles.length > 0) {
        productForm.imageFiles.forEach((f: File) => fd.append("images", f));
      } // only append urls if there are actual URL values (non-empty)
const urlImages = (productForm.images || []).filter(Boolean);
if ((productForm.imageFiles?.length ?? 0) === 0 && urlImages.length) {
  // use separate field so multer won't be confused for file upload
  fd.append("images_urls", JSON.stringify(urlImages));
      ...
      // Update local product context via addProduct (your ProductContext also persists / falls back when server not available)
      addProduct({
                ...
                <div className="flex-shrink-0 w-24 ml-4">
                  <div className="w-full bg-gray-200 rounded-full h-2">
                    <div
                      ...
                      style={{ width: `${Math.min((product.reviews / (topSellingProducts[0]?.reviews || 1)) * 100, 100)}%` }}
                    />
                  </div>
                </div>
              </div>
                ...
                .map((category) => (
                  <tr key={category.id}>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span
                        className={`px-2 py-1 text-xs font-medium rounded-full ${
                          (category as any).status === "active" ? "bg-green-100 text-green-800" : "bg-red-100 text-red-800"
                        }`}
                      >
                        {(category as any).status ? ((category as any).status.charAt(0).toUpperCase() + (category as any).status.slice(1)) : "Active"}
                      </span>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium space-x-2">
                      <button onClick={() => handleEditCategory(category.id)} className="text-blue-600 hover:text-blue-900">
                        <Edit className="h-4 w-4" />
                      </button>
                      <button onClick={() => setDeleteConfirm({ type: "category", id: category.id })} className="text-red-600 hover:text-red-900">
  ...
  // ---------- Products rendering (modal uses Portal) ----------
        ...
        <form onSubmit={handleProductSubmit} className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) => setProductForm((prev) => ({ ...prev, offerPrice: parseFloat(e.target.value || "0") }))}
                className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent"
              />
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) => setProductForm((prev) => ({ ...prev, status: e.target.value as "active" | "inactive" }))}
                className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent"
              >
                <option value="active">Active</option>
                <option value="inactive">Inactive</option>
          ...
          <div>
            ...
            {productForm.images.map((image, index) => (
              <div key={index} className="flex items-center space-x-2 mb-2">
                <input
                  ...
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent"
                  placeholder="Image URL or data preview"
                />
                <button
                  type="button"
            ...
            <div className="flex items-center space-x-3">
              <button type="button" onClick={() => setProductForm((prev) => ({ ...prev, images: [...prev.images, ""] }))} className="text-green-600 hover:text-green-700 text-sm">
                ...
                .map((product) => {
                  ...
                  return (
                    <tr key={product.id}>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="flex items-center">
                          <img className="h-10 w-10 rounded-lg object-cover mr-3" src={((product as any).images && (product as any).images[0]) || (product as any).image || ""} alt={product.name} />
                          <div>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                        <div>₹{(product as any).offerPrice || (product as any).price}</div>
                        {(product as any).offerPrice && (product as any).offerPrice < (product as any).price && (
                          <div className="text-xs text-gray-400 line-through">₹{(product as any).price}</div>
                        )}
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{(product as any).stock}</td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <span className={`px-2 py-1 text-xs font-medium rounded-full ${((product as any).status === "active") ? "bg-green-100 text-green-800" : "bg-red-100 text-red-800"}`}>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm font-medium space-x-2">
                        ...
                        <button onClick={() => setDeleteConfirm({ type: "product", id: product.id })} className="text-red-600 hover:text-red-900">
                          <Trash2 className="h-4 w-4" />
                        </button>
                      </td>
                    </tr>
