...
export default function Home() {
  ...
  // Stable addToCart so event listeners can safely include it in deps
  const addToCart = useCallback((product, incomingQty = 1) => {
    if (!product) return;
    const qtyToAdd = Math.max(1, Number.isFinite(Number(incomingQty)) ? Math.trunc(incomingQty) : 1);
    setCartItems((prev) => {
  ...
  const onProductAdd = useCallback(
    (e) => {
      try {
        ...
        const product = payload && payload.product ? payload.product : payload;
        const qty = payload && (payload.quantity ?? payload.qty) ? Number(payload.quantity ?? payload.qty) : 1;
  ...
  // openCart listener (already in your code) — opens sidebar when event fires
  useEffect(() => {
    const openHandler = () => setIsCartOpen(true);
    window.addEventListener("openCart", openHandler);
    return () => window.removeEventListener("openCart", openHandler);
