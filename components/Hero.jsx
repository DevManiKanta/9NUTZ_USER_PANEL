...
// /**
//  * Helper: turn a relative backend path into a full absolute URL using API_BASE.
//  * Accepts absolute urls and data: URIs unchanged.
//  */
...
// /**
//  * Public category fetcher (kept for the category carousel).
//  * Uses your apiAxios instance.
//  */
...
//       try {
//         // Using apiAxios so it respects any baseURL & auth you configure
//         // Try common endpoints in order for robustness
//         // Prefer 'admin/banners' (your API shows /api/admin/banners/)
//         const res = await apiAxios.get("/admin/banners");
//         if (cancelled) return;

//         const payload = res?.data ?? null;
//         // Normalize: API might return { status: true, data: [...] } or [...] etc.
...
//         // Keep only active banners (is_active truthy), but if none active keep the array as-is
...
//           // prefer redirect_url (absolute url) for display; otherwise convert image_url to full url
...
//           setBanners([]); // show no banners (no static fallback per request)
//         }
//       } finally {
...
//   // fetch categories (kept similar to your previous implementation)
//   useEffect(() => {
//     let cancelled = false;
//     (async () => {
...
//           </div>
//         ) : error ? (
//           // Error state — shows a neutral area and the error message
...
//           </div>
//         ) : banners.length === 0 ? (
//           // Empty state — no banners currently active
//           <div className="w-full h-80 md:h-96 lg:h-[28rem] bg-gray-100 flex items-center justify-center">
//             <span className="text-gray-500">No banners available</span>
//           </div>
//         ) : (
//           // Banners carousel (simple translateX slider)
//           <div
//             className="flex transition-transform duration-700 ease-in-out"
//             style={{ transform: `translateX(-${currentBanner * 100}%)`, width: `${Math.max(1, banners.length) * 100}%` }}
//           >
//             {banners.map((banner) => {
//               const bg = banner.image_url || PLACEHOLDER;
//               const redirect = banner.redirect_url || "";
//               return (
//                 <div key={banner.id} className="w-full flex-shrink-0">
//                   <div
//                     role={redirect ? "link" : "img"}
//                     tabIndex={0}
//                     onClick={() => {
//                       if (redirect) {
//                         // open in same tab (as previously)
//                         window.location.href = redirect;
//                       }
//                     }}
//                     onKeyDown={(e) => {
//                       if ((e.key === "Enter" || e.key === " ") && redirect) {
//                         e.preventDefault();
//                         window.location.href = redirect;
//                       }
//                     }}
...
//                         {/* optional CTA that uses redirect_url */}
//                         {redirect ? (
//                           <div>
//                             {/* <button
//                               onClick={(e) => {
...
//         {/* Prev / Next controls (hidden when loading or no banners) */}
    ...
    const fetchBanners = async () => {
      try {
        ...
        try {
          ...
          res = await apiAxios.get("http://192.168.29.100:8000/api/admin/banners/");
        }
