...
export default function Header({
  ...
  // When user submits search — dispatch global event and then clear input
  const handleSearchSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    const term = (query || "").trim();
    // dispatch only if something was entered (still ok to dispatch empty if you want)
    window.dispatchEvent(new CustomEvent("siteSearch", { detail: term }));
    // Clear the input and blur so UI shows cleared state
    setQuery("");
