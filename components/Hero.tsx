...
/**
 * Hero.tsx
 * - Fetches banners (same as before) from API_BASE /api/banners (no auth)
 * - Fetches categories directly from the public categories endpoint using the token you provided.
 * - Renders hero banners and a categories image carousel (react-slick).
 *
 * Important: token is present client-side per your request (for testing).
 */
...
// ---------- Put the categories API call here (client-side) ----------
const CATEGORIES_API_URL = "https://9nutsapi.nearbydoctors.in/public/api/admin/categories/show";
const CATEGORIES_API_TOKEN =
  ...
  try {
    const res = await fetch(CATEGORIES_API_URL, {
      ...
      headers: {
        ...
        Authorization: `Bearer ${CATEGORIES_API_TOKEN}`,
      },
      cache: "no-store",
    });
    if (!res.ok) {
