...
// components/allcombopacks/AllPackages.jsx
// components/allcombopacks/AllPackages.jsx
// components/allcombopacks/AllPackages.jsx
"use client";
...
import React, { useState } from "react";
import Image from "next/image";
import { useCart } from "@/contexts/CartContext"; // optional - safe fallback
import testImage from "../../assests/test.jpg";
...
export default function AllPackages({ onAddToCart: onAddToCartProp }) {
  const [selected, setSelected] = useState(null);
  const [chosen, setChosen] = useState(null);

  // safe context usage — may be undefined if provider is not present
  const cartCtx = useCart?.() ?? null;
  const addItemFromContext = cartCtx?.addItem;

  const onAddToCart = (pkg, qty = 1) => {
    // priority: prop from parent -> context -> no-op (warn)
    if (typeof onAddToCartProp === "function") {
      onAddToCartProp({
        id: pkg.id,
        name: pkg.name,
        price: pkg.priceOneTime ?? 0,
        image: typeof pkg.image === "string" ? pkg.image : undefined,
        quantity: qty,
        meta: { subtitle: pkg.subtitle },
      });
      return;
    }

    if (typeof addItemFromContext === "function") {
      addItemFromContext({
        id: pkg.id,
        name: pkg.name,
        price: pkg.priceOneTime ?? 0,
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <section aria-label="Combo packages" className="grid gap-6 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
          {PACKAGES.map((pkg) => {
            const isSelected = chosen === pkg.id;
            return (
              <article key={pkg.id} className="relative bg-white rounded-2xl shadow-md overflow-hidden border transition-transform transform hover:-translate-y-1">
                <div className="relative w-full h-44 sm:h-48 lg:h-40 xl:h-44 bg-gray-100">
                  <Image src={pkg.image} alt={pkg.name} fill style={{ objectFit: "cover" }} sizes="(max-width: 640px) 100vw, (max-width: 1024px) 50vw, 33vw" />
                </div>
                ...
                <div className="p-4 sm:p-5 flex flex-col h-full">
                  <div className="flex items-start justify-between">
                    ...
                    <div className="text-right">
                      {pkg.savingsPercent > 0 ? (
                        <div className="inline-flex items-center px-2 py-1 rounded-full bg-green-50 text-green-700 text-xs font-medium">Save {pkg.savingsPercent}%</div>
                      ) : (
                        <div className="inline-flex items-center px-2 py-1 rounded-full bg-yellow-50 text-yellow-700 text-xs font-medium">Bestseller</div>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="mt-4 mb-3 flex items-baseline gap-3">
                    <div>
                      <div className="text-2xl font-bold text-gray-900">{pkg.priceMonthly ? `${currency(pkg.priceMonthly)}/mo` : currency(pkg.priceOneTime)}</div>
                      <div className="text-xs text-gray-500">{pkg.priceMonthly ? `One-time: ${currency(pkg.priceOneTime)}` : "One-time rate"}</div>
                    </div>
                    <div className="ml-auto text-right">
                      <div className="text-sm text-gray-500">Upfront / One-time</div>
                    </div>
                  ...
                  <ul className="mt-2 mb-4 flex-1 space-y-2 text-sm text-gray-600">
                    {pkg.features.map((f, i) => (
                      <li key={i} className="flex items-start gap-2">
                        <svg className="w-4 h-4 mt-1 text-amber-600 flex-shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                          <path d="M20 6L9 17l-5-5" />
                        </svg>
                        <span>{f}</span>
                      </li>
                  ...
                  <div className="mt-3 flex items-center gap-3">
                    <button onClick={() => handleChoose(pkg)} className={`flex-1 inline-flex items-center justify-center px-4 py-2 rounded-lg font-medium shadow-sm transition ${isSelected ? "bg-rose-600 text-white" : "bg-white border border-rose-600 text-rose-600 hover:bg-rose-50"}`}>
                      {isSelected ? "Selected" : "Select"}
                    </button>

                    <button onClick={() => openDetails(pkg)} className="inline-flex items-center justify-center px-3 py-2 rounded-lg bg-gray-100 text-gray-700 text-sm hover:bg-gray-200">
                      Details
                    </button>

                    <button onClick={() => onAddToCart(pkg, 1)} className="inline-flex items-center justify-center px-3 py-2 rounded-lg bg-emerald-600 text-white text-sm hover:bg-emerald-700" title="Add to cart">
                      Add to cart
                    </button>
                  </div>
                </div>
        ...
        {selected && (
          <div className="fixed inset-0 z-40 flex items-end sm:items-center justify-center p-4 sm:p-6" role="dialog" aria-modal="true">
            <div className="absolute inset-0 bg-black/40 backdrop-blur-sm" onClick={closeDetails} aria-hidden="true" />
            <div className="relative w-full max-w-3xl bg-white rounded-2xl shadow-xl overflow-auto max-h-[85vh]">
              <div className="flex items-start gap-4 p-4 sm:p-6">
                <div className="relative w-28 h-20 rounded-lg overflow-hidden bg-gray-100 flex-shrink-0">
                  <Image src={selected.image || "/placeholder.png"} alt={selected.name} fill style={{ objectFit: "cover" }} sizes="140px" />
                </div>
                ...
                <div className="flex-1">
                  ...
                  <div className="mt-4 flex flex-col sm:flex-row sm:items-center sm:gap-6">
                    <div>
                      <div className="text-2xl font-bold">{selected.priceMonthly ? `${currency(selected.priceMonthly)}/mo` : currency(selected.priceOneTime)}</div>
                      <div className="text-xs text-gray-500">{selected.priceMonthly ? `One-time: ${currency(selected.priceOneTime)}` : "One-time rate"}</div>
                    </div>
                  ...
                  <div className="mt-6 flex items-center gap-3">
                    <button onClick={() => { onAddToCart(selected, 1); closeDetails(); }} className="px-4 py-2 rounded-lg bg-rose-600 text-white font-medium">Add to cart</button>
                    <button onClick={() => { handleChoose(selected); closeDetails(); }} className="px-3 py-2 rounded-lg bg-gray-100 text-gray-700">Choose this pack</button>
                    <button onClick={closeDetails} className="px-3 py-2 rounded-lg bg-gray-100 text-gray-700">Close</button>
                  </div>
                </div>
              </div>
            </div>
